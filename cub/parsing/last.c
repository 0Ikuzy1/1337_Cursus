...
void draw_line(t_player *player, t_game *game, float ray_angle, int column)
{
    ...
    if (DEBUG) {
        ...
        while (!touch(rayX, rayY, game) && steps < 1000) {Error: Could not get texture data\n
    ...
    void *texture_img = NULL;
    if (side == 1) {  // Horizontal wall hit
        if (rayDirY > 0) {
    ...
    char *texture_data = mlx_get_data_addr(texture_img, &tex_bpp, &tex_size_line, &tex_endian);
    ...
    int tex_width = game->textures.tex_width;
    int tex_height = game->textures.tex_height;
    ...
    for (int y = drawStart; y < drawEnd; y++) {
        ...
        int tex_index = texY * tex_size_line + texX * (tex_bpp / 8);
        ...
        if (tex_index >= 0 && tex_index < tex_size_line * tex_height - 3) {
            ...
            unsigned char b = texture_data[tex_index];
            unsigned char g = texture_data[tex_index + 1];
            unsigned char r = texture_data[tex_index + 2];
